...
################################################################################
# Extract H1-H2 (uncorrected) for wav files with associated textgrids.
...
form
	...
	text inSoundDir C:/Users/Thomas/Desktop/test
	comment Directory of input textgrid files
	text inTgDir C:/Users/Thomas/Desktop/test
	comment How large should the window be for measurements?
	real windowWidth 0.100
    comment Set the pitch range here
    real pitchMin 50
    real pitchMax 800
endform


procedure get_h1mh2: .soundObj, .regionStart
    ...
    # !!! This uses filtered autocorrelation, if you want to use
    #     raw autocorrelation, be sure to change this line accordingly.
    #     !!! Must be using praat version >6.4, as older !!! 
    #     !!! versions use different pitch commands      !!!
    # pitchObj = To Pitch (raw ac): 0, pitchMin, pitchMax, 15, "no", 0.03, 0.45, 0.01, 0.35, 0.14
    pitchObj = To Pitch (filtered ac): 0, pitchMin, pitchMax, 15, "yes", 0.03, 0.09, 0.5, 0.055, 0.35, 0.14

    # Extract f0 from the pitch object
    # (could also use Get value at time if you want a point estimate)
    ...
    if f0val <> undefined
        ...
        selectObject: .soundObj
        soundSelectionObj = Extract part: .regionStart, regionEnd, "Hamming", 1, "no"
        ...
        appendInfoLine: "F0 undefined for selected region"
    endif
endproc
...
Create Strings as file list: "list", inSoundDir$ + "*.wav"
numberOfFiles = Get number of strings

writeInfoLine: "Extracting H1-H2 for " + string$(numberOfFiles) + " files."
...
for ifile from 1 to numberOfFiles
  ...
  tgExists = fileReadable(inTgDir$ + tgFilename$)

  if wavExists and tgExists
    wavObj = Read from file: inSoundDir$ + wavFilename$
    tgObj = Read from file: inTgDir$ + tgFilename$
    
    # !!! Insert code here for extracting a timepoint from a textgrid
    #     depending on how the textgrid is structured. The example
    #     here extracts the start point of the 2nd interval on tier 2
    selectObject: tgObj
    timepointFromTextgrid = Get start time of interval: 2, 2
    
    # Get H1-H2
    @get_h1mh2: wavObj, timepointFromTextgrid

    # Extract result
    h1mh2 = get_h1mh2.h1mh2


    # !!! do something with h1mh2 here
    appendInfoLine: objName$ + " = " + string$(h1mh2)

    # Cleanup
    selectObject: wavObj, tgObj
    Remove
  else
    appendInfoLine: "File not found for: " + objName$
  endif
endfor
